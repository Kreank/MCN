...
| I9 | ~~Mehr Infos am Raum gewünscht.~~ **Geschlossen 2026-07-21:** Sascha kann die Notiz nicht mehr einordnen. Vorhanden sind Etage, Raumtyp, Fläche, Höhe, Volumen (generiert), Umfang, Innentemperatur, Luftwechsel, Heizlast-Kennwert, Steigleitungsabstand, Notiz — **keine Raumnummer**. Fällt im nächsten Praxistest wirklich etwas, kommt es konkret zurück. | — | `0086_raumaufmass.py:136-192` | **verworfen** |
| I10 | Räume sollen Belegungs-Infos mit anzeigen (heute nur über Belegung erreichbar). | UI | — | offen |
| I11 | „Mehr Logik bei Struktur-/Raumzuweisung" — **Klarstellung Sascha 2026-07-21: kein Feature-Wunsch, sondern ein Prüfauftrag.** Die Zuweisungslogik selbst durchgehen und beurteilen. **Ergebnis: nicht zufrieden.** Das System fragt an vier Stellen, was es bereits weiß. Analyse und Umsetzung siehe I11a–I11f. | — | — | **analysiert + teilweise umgesetzt** |
| I11a | **Die Etage wurde nicht von der Wohnung geerbt.** `unit.storey` gibt es seit 0124, der Raum-Editor hat die Einheiten samt Etage im Speicher — und tippte sie nicht ein. Bei jedem Raum jeder Wohnung erneut „3. OG". Leer gelassen fällt der Raum in die Sammelgruppe „Ohne Geschossangabe", also aus der einzigen Ordnung, die die Raumliste hat. **Nur bei leerem Feld übernehmen** (Maisonette; getippte Angaben gehören dem Anwender). Die Einheitsauswahl zeigt die Etage jetzt mit an. | UI | `raum-editor.ts` | **umgesetzt 2026-07-21** |
| I11b | **Das Gebäude wurde nicht aus der Einheit abgeleitet** — obwohl `ensure_standort` den Wert an derselben Stelle bereits las, um ihn zu vergleichen, und dann „Eine Einheit setzt ein Gebäude voraus" warf. Die Redundanz war eine Bringschuld **jedes** Aufrufers und leckte als **dreifach kopierte Auswahlkaskade** ins Frontend (Raum-Editor, Anlagen-Dialog, Plantafel — alle mit dem Kommentar „Vorbild anlage-dialog"). `ensure_standort` gibt jetzt `(building_id, unit_id)` zurück; Raum- und Anlagen-Service schreiben den abgeleiteten Wert. Die Konsistenzprüfung bleibt scharf: ein **falsches** Gebäude wird weiter abgewiesen, nur ein **fehlendes** ergänzt. | UI | `services/_validation.py`, `raum.py`, `anlage.py` | **umgesetzt 2026-07-21** |
| I11c | **Kein Auto-Select bei genau einer Option.** Eine Liegenschaft mit einem Gebäude und einer Einheit verlangte trotzdem zwei Klicks auf Dropdowns mit je einem echten Eintrag — Vorauswahl „keine Zuordnung (Altbestand)". Das Muster „ein Treffer ⇒ vorbelegen" existiert im Repo schon dreimal (`ids-bestellung`, `auftrag-detail`, `bericht-positionen`), war auf die Objektstruktur aber nie angewandt. Nur beim **neuen** Raum und nur ohne Vorbelegung: „kein Gebäude" ist ein gültiger, bewusst wählbarer Zustand. | UI | `raum-editor.ts` | **umgesetzt 2026-07-21** |
| I11d | **Keinerlei Serienanlage.** „EG bis 4. OG, je vier Wohnungen" = 20 Dialogdurchläufe, 40 Pflichteingaben, davon 20-mal derselbe Einheitstyp. Der Dialog schloss nach jedem Anlegen und setzte **alle** Felder zurück. Umgesetzt ist die billige Hälfte: **„Speichern und noch eine"** hält Typ und Etage, leert nur die Nummer und setzt den Fokus zurück. Ein echter Serien-Dialog („Etagen × Einheiten × Nummernschema") bräuchte einen Bulk-Endpunkt mit Vorschau und Kollisionsbehandlung gegen `UNIQUE (property_id, unit_number)` — eigenes Paket. | UI | `liegenschaft-detail.ts` | **teilweise umgesetzt** |
| I11e | **`EINFAMILIENHAUS` ist ein Etikett ohne Wirkung auf die Struktur.** Beim laut Anruf-Dialog häufigsten Objekttyp wird die volle Hierarchie Gebäude → Einheit → Raum abgefragt, obwohl beide determiniert sind. Der Typ tut funktional genau an **einer** Stelle etwas (im Anruf-Dialog entfällt die Eigentumsfrage). Zwei Lösungswege mit unterschiedlichen Folgekosten; verknüpft mit der Frage, wie sich `property_type` überhaupt ändern lässt. **Nach I11a–c angehen, nicht davor.** | MODELL | `0048_property_type_efh.py` | **offen** |
| I11f | **Größter Fund des Prüfauftrags, außer der Reihe:** `vorgang.building_id/unit_id` (0012) und `auftrag.building_id/unit_id` (0013) haben **vollen zusammengesetzten Fremdschlüssel in der DB und keine einzige Zeile API oder UI**. Ein Auftrag lässt sich heute **nicht auf eine Wohnung legen** — „Klo kaputt, 3. OG rechts" muss in den Freitext. Dasselbe Muster wie J5 und I1: fertige Datenbank, nie angebundene Oberfläche. Für den Disponenten unmittelbar spürbar. **Eigenes Arbeitspaket in der Größenordnung von J5.** | UI | `0012_vorgang.sql:20,39`, `0013_auftrag.sql:19,49` | **offen — groß** |
...
**Umgesetzt 2026-07-21 (Backend):** Migration **0126** zieht den Schutzstandard auf sieben
Tabellen nach; neu sind `PATCH` für Kommunikationsweg, Adresszuordnung (plus `/ersetzen`
und `/beenden`), Person, Organisation und Liegenschaft sowie `include_ended` an zwei Listen.

| # | Beim Bau gefunden | Status |
|---|---|---|
| H9 | **Falle, die ein Review fand:** `person` und `organization` haben als PK `party_id`, nicht `id`. Der Standard-Audit-Trigger schriebe dort stumm `NULL` als `target_id` — das Audit wüsste nicht, WEN es protokolliert. Dafür gibt es jetzt `audit.audit_row_update_key()` mit der Schlüsselspalte als Trigger-Argument. | behoben |
| H10 | **Die Sackgasse aus F4a, empirisch bestätigt:** `excl_party_address_primary` schließt sich über den Gültigkeitszeitraum. Eine heute beendete Zeile belegt `[heute, morgen)` und überlappt die neue `[heute, )` weiterhin — „beenden, dann richtig anlegen" endete in einem 422, das eine **bereits beendete** Adresse als Konflikt meldete. Lösung: Eine am selben Tag zurückgenommene Zuordnung verliert ihr **Primär-Kennzeichen** — eine Primäradresse, die keinen Tag galt, war nie eine. Gleiche Falle beim Kommunikationsweg (dort vorbestehend), gleiche Lösung. | behoben |
| H11 | **Latent, nicht behoben:** `ersetze_party_address` und `update_property` hängen nur ihre eigene Zeile um. `billing.billing_instruction.billing_address_id` (`0007_billing.sql:16`) und `building.address_id` (`0004_property.sql:62`) zeigen danach weiter auf die alte Adresse. Heute folgenlos (kein Python-Pfad schreibt `billing_address_id`), aber beim Anbinden der Abrechnungsanweisung zu bedenken. | **offen — vormerken** |
| H12 | **Toter Testzweig, gefunden im Review:** Der erste No-Delete-Test enthielt `(Property, None)` mit `continue` — er *sah aus*, als prüfe er die Liegenschaft, und übersprang sie. Ausgerechnet `property` und `party`, die beiden Trigger mit der größten Reichweite, waren ungetestet. **Merksatz: Ein `continue` in einer Prüfschleife ist ein blinder Fleck, der wie Abdeckung aussieht.** | behoben |
...
| ~~I9~~ | ~~Welche Raum-Infos fehlen konkret?~~ **Verworfen 2026-07-21** — Notiz nicht mehr einzuordnen. | Erledigt. Fällt im nächsten Test wirklich etwas, kommt es konkret zurück. |
| ~~I11~~ | ~~„Mehr Logik bei Struktur-/Raumzuweisung" — was genau?~~ **Keine offene Frage an Sascha**: Er hat 2026-07-21 klargestellt, dass es ein **Prüfauftrag an den Entwickler** ist. | Analyse und Vorschläge liefere ich, keine Rückfrage nötig. |
