...
type ViewState =
  | { kind: 'loading' }
  | { kind: 'ready'; data: Trade[] }
  | VerbotenState
  | { kind: 'error' };

/**
 * Gewerk-Katalog (company.trade) — die zentrale Gewerk-Wahrheit. Anlegen,
 * Bezeichnung ändern, Deaktivieren (kein Löschen). Ohne `company/AENDERN`
 * schreibgeschützt.
 */
@Component({
  selector: 'app-gewerke',
  imports: [ReactiveFormsModule, Feld, EinstellungenNav, KeinZugriff],
  templateUrl: './gewerke.html',
  styleUrl: './gewerke.scss',
})
export class Gewerke {
  ...
  anlegen(): void {
    ...
    this.laedt.set(true);
    this.svc.createTrade({ code: v.code.trim().toUpperCase(), label: v.label.trim() }).subscribe({
      next: () => {
      ...
      error: (err: unknown) => {
        this.laedt.set(false);
        this.meldung.set(fehlerDetail(err) ?? 'Das Gewerk konnte nicht angelegt werden.');
      },
    });
  }

  starteBearbeiten(t: Trade): void {
    if (!this.darfAendern()) return;
    this.meldung.set(null);
    this.editLabel.setValue(t.label);
    this.bearbeite.set(t.id);
  }

  speichern(t: Trade): void {
    if (this.laedt()) return;
    this.editLabel.markAsTouched();
    if (this.editLabel.invalid) return;
    this.laedt.set(true);
    this.svc.updateTrade(t.id, { label: this.editLabel.value.trim() }).subscribe({
      next: () => {
        this.laedt.set(false);
      ...
      error: (err: unknown) => {
        this.laedt.set(false);
        this.meldung.set(fehlerDetail(err) ?? 'Das Gewerk konnte nicht gespeichert werden.');
      },
    });
  }

  umschalten(t: Trade): void {
    if (this.laedt() || !this.darfAendern()) return;
    this.meldung.set(null);
    this.laedt.set(true);
    this.svc.updateTrade(t.id, { active: !t.active }).subscribe({
      next: () => {
        this.laedt.set(false);
        this.laden();
      },
